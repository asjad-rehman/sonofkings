/* Son of Kings storefront — dependency-free, multi-color ordering */
(function () {
  'use strict';
  ...
  var SITE={
    ...
    colors:{
      Black:{front:'assets/img/black-hoodie.webp',back:'assets/img/black-trousers.webp'},
      Navy:{front:'assets/enhanced/navy-hoodie.webp',back:'assets/enhanced/navy-pants.webp'},
      'Royal Blue':{front:'assets/enhanced/blue-hoodie.webp',back:'assets/enhanced/blue-pants.webp'},
      'Forest Green':{front:'assets/enhanced/green-hoodie.webp',back:'assets/enhanced/green-pants.webp'},
      Marble:{front:'assets/enhanced/marble-hoodie.webp',back:'assets/enhanced/marble-pants.webp'},
      Crimson:{front:'assets/enhanced/red-hoodie.webp',back:'assets/enhanced/red-pants.webp'}
    }
  };
  var KEY='sok.bag.v2', bag=load(), chosenSize='M', chosenColor='Black', qty=1;

  function key(c,s){return c+'|'+s}
  function load(){try{var x=JSON.parse(localStorage.getItem(KEY)||'{}'),o={};Object.keys(x).forEach(function(k){var p=k.split('|'),q=parseInt(x[k],10);if(SITE.colors[p[0]]&&SITE.sizes.indexOf(p[1])>-1&&q>0)o[k]=Math.min(q,SITE.max)});return o}catch(e){return {}}}
  ...
  /* Color selection: existing studio cards become real product selectors. */
  var cards=$$('.edition-card');
  cards.forEach(function(card){
    var h=$('h3',card);if(!h||!SITE.colors[h.textContent.trim()])return;
    var c=h.textContent.trim();card.dataset.color=c;card.classList.add('edition-card--available');card.tabIndex=0;card.setAttribute('role','button');card.setAttribute('aria-label','Select '+c+' tracksuit');
    var body=$('.edition-card__body',card), old=body&&body.querySelector('span');if(old)old.textContent='Select color';
    var stock=$('.stock',card);if(stock){stock.className='stock stock--live';stock.innerHTML='<span></span>Available now'}
    function choose(){setColor(c);var product=$('#product');if(product)product.scrollIntoView({behavior:'smooth',block:'start'})}
    on(card,'click',function(e){if(e.target.tagName==='A'&&c==='Black')e.preventDefault();choose()});on(card,'keydown',function(e){if(e.key==='Enter'||e.key===' '){e.preventDefault();choose()}})
  });

  function setColor(c){
    if(!SITE.colors[c])return;chosenColor=c;var ph=SITE.colors[c];setPlate('front',ph.front,c);setPlate('back',ph.back,c);showView('front');
    ...
    var add=$('#add-to-bag');if(add)add.textContent='Add '+c+' set — '+money(SITE.price);
    ...
    var bar=$('#buybar-size');if(bar)bar.textContent=c+' · Size '+chosenSize;
  }

  /* Size / quantity */
  var sizeBtns=$$('.size');
  function setSize(s){chosenSize=s;sizeBtns.forEach(function(b){var a=b.dataset.size===s;b.setAttribute('aria-checked',a?'true':'false');b.tabIndex=a?0:-1});var x=$('#size-current');if(x)x.textContent=s;var bar=$('#buybar-size');if(bar)bar.textContent=chosenColor+' · Size '+s}
  ...
  function add(n){var k=key(chosenColor,chosenSize),cur=bag[k]||0,next=Math.min(cur+n,SITE.max);if(next===cur)return;bag[k]=next;save();renderBag();setBag(true)}
  ...
  /* Turn preview language into live inventory language. */
  var eh=$('.editions__head .lede');if(eh)eh.textContent='Choose your color. Every color shown below is an active Son of Kings tracksuit option and can be added to your order.';
  var et=$('#editions-title');if(et)et.innerHTML='Choose your <em>color.</em>';
  $$('.faq__list details').forEach(function(d){var s=$('summary',d);if(s&&/Black the only edition/i.test(s.textContent)){s.textContent='Which colors are available?';var p=$('p',d);if(p)p.textContent='Black, Navy, Royal Blue, Forest Green, Marble and Crimson are available to select and order.'}if(s&&/What is included/i.test(s.textContent)){var p2=$('p',d);if(p2)p2.textContent='One hoodie and one matching pair of trousers in your selected color. They are sold together as a set.'}});
  ...
  var year=$('#year');if(year)year.textContent=new Date().getFullYear();setColor('Black');setSize('M');setQty(1);renderBag();
})();